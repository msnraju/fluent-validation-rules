...
export class PostMemberValidatorBuilder<T, TType>
  implements IPostMemberValidatorBuilder<T> {
  ...
  ruleFor<T2>(
    member: string,
    accessor: MemberFunc<T, T2>
  ): IMemberValidatorBuilder<T, T2> {
  ...
  withMessage<T2>(func: WithMsgFuncArg<T, T2>): IValidationBuilder<T> {
