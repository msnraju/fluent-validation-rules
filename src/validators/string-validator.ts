...
import { MemberFunc, ValidatorFunc, STRING_TYPES } from "../types";
...
export class StringValidator<T> implements IStringValidator {
    ...
    hasLength(min: number, max: number) {
        this.rule.message = () => `Length of ${this.rule.member} should be minimum ${min} and maximum ${max}.`;
    ...
    matches(expr: RegExp) {
        this.rule.message = () => `${this.rule.member} is not matching the expression.`;
